function PasswordValid (cadena, number) {
...
for (word of arr){
    ...
    let posequal = word.indexOf("-");
    let posspace = word.indexOf(" ");
    let arrmin = [], arrmax = [];
    for(let i =0; i<= posequal -1; i++){
        arrmin.push(arrWord[i]);
    }

    for(let i =posequal +1; i<= posspace -1; i++){
        arrmax.push(arrWord[i]);
    }
    let min = arrmin.join('');
    let max = arrmax.join('');
    let contador = 0;
    let pos = arrWord.indexOf(":");
    let letter = arrWord[pos-1];
    let arrfi = [];
    for(let i = pos+2; i<arrWord.length; i++){
        arrfi.push(arrWord[i]);
    }
    for(letra of arrfi){
        if (letra === letter){
            contador++;
    ...
    contador <= max && contador >= min ? results.push({bool: true, value: arrfi.join('')}) : results.push({bool: false, value: arrfi.join('')}) ;
}
let falsesResuts = [];
for(result of results){
    if(!result.bool){
        falsesResuts.push(result);
    }
}
console.log(falsesResuts[number -1].value);
}

let data = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
